...
```{r, echo=FALSE, fig.cap="A tweet from Dale Barr", out.width="500px"}
knitr::include_graphics(path="./images/Dale_tweet.png")
```
...
```{r, echo=FALSE, fig.cap="A tweet from Steph Allan", out.width="500px"}
knitr::include_graphics(path="./images/Steph_tweet.png")
```
...
```{r, echo=FALSE, fig.cap="Setting your working directory", out.width="75%", out.height="75%"}
knitr::include_graphics(path="./images/setwd.gif")
```
...
```{r code demo 1, eval=FALSE}
experiment_data <- read_csv('experiment_data.csv')
```

#### <span style="color:DARKGREEN"> Quickfire Questions</span>

* Which of the following is most likely to be an argument? `r mcq(sample(c(answer = "35", "read_csv()", "<-")))`

* An easy way to spot functions is to look for `r mcq(sample(c(answer = "brackets", "numbers", "computers")))`.

* The job of `<-` is to send the output from the function to a/an `r mcq(sample(c(answer = "variable", "assignment", "argument")))`.
