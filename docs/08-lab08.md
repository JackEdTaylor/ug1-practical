...
**Probability Recap** Binomial distributions are based on scenarios where the outcome can only be 1 of 2 options e.g. heads or tails. They can tell us, for example, the probability of observing any number of heads occurring if a coin is flipped a given number of times.

### Normal Distributions
A normal distribution looks like this:

![**Figure 8.1 Normal Distribution of height. $\mu$ = the mean (average), $\sigma$ = standard deviation**](images/norm_dist_height.png)

As we've already seen, binomial distributions show us the probability of an event occurring when there are two possible outcomes. The normal distribution, on the other hand, reflects the probability of any value occurring for a continuous variable. Examples of continuous variables include height or age where a single person can score anywhere along a continuum. For example, a person could be 21.5 years old and 176cm tall.

Normal distributions are symmetrical, meaning there is an equal probability of observations occurring above and below the mean. This means that, if the mean in figure 1 is 170, we could expect the number of people who have a height of 160 to equal the number of people who have a height of 180.

Another point worth noting is that in a normal distribution, the mean, median, and mode are all equal.
...
With the binomial distribution in lab 7 we plotted the number of successes, heads or tails, we achieved with a certain number of throws so we use a bar plot e.g. how many heads we got between 1 and 10 tosses of the coin. This time with our height example above, we are plotting the probability of observing people with heights between two points, ~140cm to ~200cm. For this, we can use a density plot (as in figure 1), or a histogram (see below), to reflect the number of people we actually observe with a given height.

<div class="figure" style="text-align: center">
<img src="08-lab08_files/figure-html/unnamed-chunk-2-1.png" alt="The distribution of heights we might actually observe in a sample of 10000 people." width="100%" />
<p class="caption">(\#fig:unnamed-chunk-2)The distribution of heights we might actually observe in a sample of 10000 people.</p>
</div>
